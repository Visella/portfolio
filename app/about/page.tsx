...
export default function AboutPage() {
  ...
  const educationItems = (profile.education || []).map((e: any) => ({
    ...
    location: e.location,
    date: e.date,
    bullets: [e.degree ? `${e.degree}${e.gpa ? ` · GPA ${e.gpa}` : ""}` : ""].filter(Boolean),
  }))
  ...
  return (
    <main className="min-h-[100dvh]">
      ...
      <section className="mx-auto w-full max-w-6xl px-4 pb-24 pt-24">
        {/* Centered page title */}
        <h1 className="text-center text-3xl font-semibold text-primary">About</h1>
        <p className="mt-2 text-center text-muted-foreground">Skills, journey, and achievements.</p>
          ...
          <Card className="border-border/60 bg-card/60 backdrop-blur">
            ...
            <CardContent className="space-y-4">
              {profile.achievements.map((a, i) => (
                <div key={i} className="grid grid-cols-[100px_1fr] gap-3 md:grid-cols-[140px_1fr]">
                  <Image
                    ...
                    onClick={() =>
                      (window as any).dispatchEvent(
                        new CustomEvent("open-achievement", { detail: a.image || "/placeholder.jpg" }),
                      )
                    }
                  />
                  <div>
